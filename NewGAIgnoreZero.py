...
p_ran = m_ran = sys.argv[3]
p_uni = m_uni = sys.argv[4]
nb_run = sys.argv[5]
...
while index < int(index_range):
    ...
    for idRun in range(int(nb_run)):
        ...
        population = [individual(w=0, p_ran=float(p_ran), p_uni=float(p_uni)) for _ in range(population_size)]
...
row = {"Lan Chay": "Average", "Time": sum_time / int(index_range), "Co Sac": sum_lifetime / int(index_range),
           "Khong Sac": min([E[j] / e[j] for j, _ in enumerate(node_pos) if e[j] > 0])}
writer.writerow(row)
